# SindbadForwardDiffExt

**Trigger package**: `ForwardDiff`

This extension is loaded automatically by Julia when `ForwardDiff` is available.

**Source**: [`ext/SindbadForwardDiffExt/SindbadForwardDiffExt.jl`](https://github.com/LandEcosystems/Sindbad/blob/main/ext/SindbadForwardDiffExt/SindbadForwardDiffExt.jl)

## Includes

- [`MachineLearningGradientSite.jl`](https://github.com/LandEcosystems/Sindbad/blob/main/ext/SindbadForwardDiffExt/MachineLearningGradientSite.jl)
- [`SindbadForwardDiffExt.jl`](https://github.com/LandEcosystems/Sindbad/blob/main/ext/SindbadForwardDiffExt/SindbadForwardDiffExt.jl)
